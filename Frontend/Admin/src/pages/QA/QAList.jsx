...
import QAListTable from "../../components/QA/QAListTable";
...
const QAList = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setShowSuccessMessage({
        success: true,
        message: "Xóa Hỏi đáp thành công.",
      });
      setTimeout(
  ...
  const handleSelectAll = (isChecked) => {
    if (isChecked) {
      const allIds = searchedQAs.map((QA) => QA.id);
      setSelectedQAIds(allIds);
    } else {
      setSelectedQAIds([]);
    }
  };
  ...
  const fetchQAs = async () => {
    try {
      const token = getUserToken();
      const response = await axios.get("http://localhost:8081/api/qas", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const searchedQAs = QAs.filter((QA) => {
    const qaId = QA.QAId?.toLowerCase() || "";
    const productName = productsMap[QA.productId]?.toLowerCase() || "";
    const username = users[QA.userId]?.toLowerCase() || "";

    return (
      qaId.includes(searchQuery.toLowerCase()) ||
      username.includes(searchQuery.toLowerCase()) ||
      productName.includes(searchQuery.toLowerCase())
    );
  });

  const totalQAs = searchedQAs.length;

  const indexOfLastQA = currentPage * QAsPerPage;
  const indexOfFirstQA = indexOfLastQA - QAsPerPage;

  const currentQAs = searchedQAs.slice(indexOfFirstQA, indexOfLastQA);

  const totalPages = Math.ceil(searchedQAs.length / QAsPerPage);
      ...
      <div className="mt-4 w-full relative shadow-md sm:rounded-lg">
        ...
        <div className="flex items-center justify-between flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4">
         ...
         dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Nhập tên người hỏi hoặc sản phẩm"
                required
              />
        ...
        <div className="mt-4 p-4 bg-gray-100 rounded-lg shadow dark:bg-gray-800">
          ...
          <QAListTable
            ...
            isAllSelected={
              selectedQAIds.length === searchedQAs.length &&
              searchedQAs.length > 0
            }
