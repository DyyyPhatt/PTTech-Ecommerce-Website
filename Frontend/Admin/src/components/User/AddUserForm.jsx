  ...
  return (
    <>
      ...
      >
        {showErrorMessage && (
          <div
            className="fixed top-4 right-4 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 dark:text-gray-300 bg-white dark:bg-gray-800 rounded-lg shadow-lg"
            role="alert"
          >
            ...
            {rolesList.map((role) => (
              <div key={role.id} className="flex items-center space-x-2">
                <input
                  ...
                  name="roles"
                  value={role.id}
                  checked={user.roles.some((r) => r.roleName === role.label)}
                  onChange={handleRoleChange}
                  className="h-4 w-4 text-blue-500 border-gray-300 dark:border-gray-600 rounded"
                />
                <label
                  htmlFor={role.id}
